...
open(F3,">".$ref_seq."_5_Positive_Coupling");
open(F4,">".$ref_seq."_5_Negative_Coupling");
...
for ($i = 1; $i <= $seq_len; $i++) {
	...
	for ($j = 1; $j <= $seq_len; $j++) {
		...
		print F4 log($K_fj_pertk)," ";
		#print $i," ",$F1," ",$U1," ",$j," ",$F2," ",$U2," ",$numerator," ",$denominator," ",$K_fj_pertk, " ",log($K_fj_pertk),"\n";	
			}
		print F4 "\n";
		}
close(F4);
print "\nPlease check the ".$ref_seq."_5_Positive_Coupling for Positive Coupling\n";
print "\nPlease check the ".$ref_seq."_5_Negative_Coupling for Negative Coupling\n";
